...
// #define MY_FILE "./B/file"
// #define MY_FILE1 "./B/file1"
// #define MY_FILE2 "./B/file2"
// #define MY_TMP_FILE "./B/file_tmp"
...
int main(int argc, char *argv[])
{
    ...
    int fd0 = -1, fd1 = -1;
    if ((fd0 = open(MY_FILE2, O_RDWR)) == -1 || (fd1 = open(MY_FILE1, O_RDWR)) == -1)
    {
        perror("Open");
        return 1;
    }
    ...
    {
        if (ftruncate(fd1, (65 + round) * BUF_SIZE) != 0)
        {
            perror("Ftruncate fd1");
            return 1;
        }
        void *buf0 = (char *)mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd0, (63 - round) * BUF_SIZE);
        void *buf1 = (char *)mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd1, (64 + round) * BUF_SIZE);
        ...
        printf("Round %d\n", round);
        memcpy(shmp_B_buf, buf0, BUF_SIZE);
        shmp_B_state[round] = 1;
        printf("Copied to shared memory, waiting for Alice to share his data\n");
        while (shmp_A_state[round] != 1)
            usleep(100);
        printf("Alice has shared his data, copying to file\n");
        memcpy(buf1, shmp_A_buf, BUF_SIZE);
        shmp_A_state[round] = 2;
        printf("Copied to file, waiting for Alice to copy his data\n");
        while (shmp_B_state[round] != 2)
            usleep(100);
    ...
    {
        printf("Round %d\n", 64 + round);
        memcpy(shmp_B_buf, buf + round * BUF_SIZE, BUF_SIZE);
        shmp_B_state[64 + round] = 1;
        printf("Copied to shared memory, waiting for Alice to share his data\n");
        while (shmp_A_state[64 + round] != 1)
            usleep(100);
        printf("Alice has shared his data, copying to file\n");
        memcpy(buf + round * BUF_SIZE, shmp_A_buf, BUF_SIZE);
        shmp_A_state[64 + round] = 2;
        printf("Copied to file, waiting for Alice to copy his data\n");
        while (shmp_B_state[64 + round] != 2)
            usleep(100);
    }
    ...
    buf = (char *)mmap(NULL, 64 * BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 64 * BUF_SIZE);
    if (buf == MAP_FAILED)
    {
        perror("Mmap");
        return 1;
    ...
    for (int i = 0; i < 32; ++i)
    {
        memcpy(shmp_B_buf, buf + i * BUF_SIZE, BUF_SIZE);
        memcpy(buf + i * BUF_SIZE, buf + (63 - i) * BUF_SIZE, BUF_SIZE);
        memcpy(buf + (63 - i) * BUF_SIZE, shmp_B_buf, BUF_SIZE);
    }
    ...
    fd0 = -1, fd1 = -1;
    if ((fd0 = open(MY_FILE1, O_RDWR)) == -1 || (fd1 = open(MY_TMP_FILE, O_RDWR | O_CREAT, 0666)) == -1)
    {
        perror("Open");
        return 1;
    }
    for (int round = 0; round < 128; ++round)
    {
        ...
        void *buf0 = (char *)mmap(NULL, BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd0, (127 - round) * BUF_SIZE);
        ...
        if (ftruncate(fd0, (127 - round) * BUF_SIZE) != 0)
        {
            perror("Ftruncate fd0");
            return 1;
        }
    ...
    if ((fd0 = open(MY_TMP_FILE, O_RDWR)) == -1 || (fd1 = open(MY_FILE, O_RDWR | O_CREAT, 0666)) == -1)
    {
        perror("Open");
        return 1;
    }
